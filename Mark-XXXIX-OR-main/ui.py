...
class SetupOverlay(QWidget):
    done = pyqtSignal(str, str, str, str)  # claude, os, el_key, el_voice
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("CLAUDE OR NVIDIA API KEY", 8, color=C.TEXT_DIM,
                               align=Qt.AlignmentFlag.AlignLeft))
        self._key_input = QLineEdit()
        self._key_input.setEchoMode(QLineEdit.EchoMode.Password)
        self._key_input.setPlaceholderText("sk-ant-… or nvapi-…")
        ...
        layout.addWidget(_lbl("ELEVENLABS API KEY  (optional)", 8, color=C.TEXT_DIM,
                       align=Qt.AlignmentFlag.AlignLeft))
        self._el_key_input = QLineEdit()
        self._el_key_input.setEchoMode(QLineEdit.EchoMode.Password)
        self._el_key_input.setPlaceholderText("sk_…  (leave blank to skip)")
        self._el_key_input.setFont(QFont("Courier New", 10))
        self._el_key_input.setFixedHeight(32)
        self._el_key_input.setStyleSheet(f"""
            QLineEdit {{
                background: #000d12; color: {C.TEXT};
                border: 1px solid {C.BORDER}; border-radius: 3px; padding: 4px 8px;
            }}
        ...
        """)
        layout.addWidget(self._el_key_input)

        layout.addWidget(_lbl("ELEVENLABS VOICE ID  (optional)", 8, color=C.TEXT_DIM,
                       align=Qt.AlignmentFlag.AlignLeft))
        self._el_voice_input = QLineEdit()
        self._el_voice_input.setPlaceholderText("e.g. kPzsL2i3teMYv0FxEYQ6")
        self._el_voice_input.setFont(QFont("Courier New", 10))
        self._el_voice_input.setFixedHeight(32)
        self._el_voice_input.setStyleSheet(f"""
            QLineEdit {{
                background: #000d12; color: {C.TEXT};
                border: 1px solid {C.BORDER}; border-radius: 3px; padding: 4px 8px;
            }}
            QLineEdit:focus {{ border: 1px solid {C.ACC}; }}
        """)
        layout.addWidget(self._el_voice_input)
    ...
    def _submit(self):
        ...
        el_voice = self._el_voice_input.text().strip()
        if not key:
            ...
            return
        self.done.emit(key, self._sel_os, el_key, el_voice)
...
class MainWindow(QMainWindow):
    ...
    def _check_config(self) -> bool:
        ...
        try:
            ...
            # Accept nvidia_api_key OR claude_api_key as the AI key
            has_ai_key = bool(d.get("nvidia_api_key")) or bool(d.get("claude_api_key"))
            return has_ai_key and bool(d.get("os_system"))
        except Exception:
            return False
    ...
    def _on_setup_done(self, key: str, os_name: str, el_key: str = "", el_voice: str = ""):
        os.makedirs(CONFIG_DIR, exist_ok=True)
        try:
            data = json.loads(API_FILE.read_text(encoding="utf-8")) if API_FILE.exists() else {}
        except Exception:
            data = {}
        # Auto-detect key type: nvapi- = NVIDIA NIM, otherwise Anthropic Claude
        if key.startswith("nvapi-"):
            data["nvidia_api_key"] = key
        else:
            data["claude_api_key"] = key
        data["os_system"] = os_name
        if el_key:
            data["elevenlabs_api_key"] = el_key
        if el_voice:
            data["elevenlabs_voice_id"] = el_voice
...
class JarvisUI:
    def __init__(self, face_path: str, size=None):
        ...
        # Thread-safe queue for text input from the UI send button
        import queue as _queue
        self._text_queue: _queue.Queue = _queue.Queue()
        self._win.on_text_command = self._text_queue.put

    @property
    def text_input_ready(self) -> bool:
